...
% dimensiones; datos linealmente separables (modificación).
...
labels = ones(num, 1);
labels(1:num/2) = -1;
labels(13) = 1; % Cambiamos una etiqueta para ver cómo afecta al margen
...
title("Datos linealmente separables (modificados)")
...
text(0.55, 1.45, sprintf('w''x + b = 1'), 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left', 'Color', 'b');
text(0.2, 1.05, sprintf('w''x + b = -1'), 'VerticalAlignment', 'top', 'HorizontalAlignment', 'left', 'Color', 'g');
...
legend('Clase -1', ...
    ...
    Location='northwest');
hold off;
title("Hiperplano separador obtenido con SVM de margen duro sobre datos linealmente separables (modificados)")